...
class CLIPdatasetclass:  # This class is used to achieve parameters sharing among datasets
    def __init__(self, train_file, tokenizer, max_len, savedir, test_file=None, type=None):
...
def extract_feat(datadir, savefolder, cashedir, prefix):
    ...
    if prefix != 'memotion3':
        ...
        with open(os.path.join(datadir, prefix + "_train.json"), encoding="utf8") as json_file:
            traindict = json.load(json_file)

        ## load dev features
        with open(os.path.join(datadir, prefix + "_test.json"), encoding="utf8") as json_file:
        ...
        with open(os.path.join(datadir, prefix, "train.json"), encoding="utf8") as json_file:
            traindict = json.load(json_file)
    ...
    CLIPdatasetclass(train_file=testdict,
                     tokenizer=tokenizer,
                     max_len=max_len,
                     savedir=savedir)
