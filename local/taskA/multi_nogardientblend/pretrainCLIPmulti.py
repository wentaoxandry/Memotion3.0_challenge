...
def training(config, dataset=None):
    ...
    model = CLIPmultipretrain(config["MODELtext"], config["cachedir"])
    ...
    loaded_state = torch.load(config["pretrainedtext"], map_location=config["device"])
    ...
    loaded_state = {k.replace('cliptext.', 'clipmulti.'): v for k, v in loaded_state.items() if
                    k.replace('cliptext.', 'clipmulti.') in self_state and not k.replace('cliptext.', 'clipmulti.').startswith(
                        'linear')}
    self_state.update(loaded_state)
    model.load_state_dict(self_state)

    loaded_state = torch.load(config["pretrainedimage"], map_location=config["device"])
    if isinstance(loaded_state, torch.nn.DataParallel):
        loaded_state = loaded_state.module
    loaded_state = loaded_state.state_dict()
    self_state = model.state_dict()
    ...
    loaded_state = {k.replace('clipimage.', 'clipmulti.'): v for k, v in loaded_state.items() if
                    k.replace('clipimage.', 'clipmulti.') in self_state and not k.replace('cliptext.','clipmulti.').startswith(
                        'linear')}
    self_state.update(loaded_state)
    model.load_state_dict(self_state)
...
def main(datadir, savedir, cashedir, bestmodelinfo):
    ...
    for mode in ['text', 'image']:
        modeldir = os.path.join(savedir, mode,
                                         'model')
    ...
    resultsdir = os.path.join(savedir, 'multi', 'results')
    ...
    config = {
        ...
        "pretrainedtext": pretraineddirs[0],
        "pretrainedimage": pretraineddirs[1]
    }
