...
class CLIPimagepretrain(torch.nn.Module):
    def __init__(self, modelname, cachedir, dropout, type=None):
        torch.nn.Module.__init__(self)
        self.clipimage = CLIPVisionModel.from_pretrained(modelname, cache_dir=cachedir)
        self.linearimage = torch.nn.Linear(768, 2)
    ...
    def forward(self, image):
        ...
        pooled_output = x.pooler_output
        pooled_output = self.dropout(pooled_output)
        logits = self.linearimage(pooled_output)
        return logits
