...
def eval(config, dataset=None):
    ...
    model = CLIPmulti(config["MODELtext"], config["cachedir"], config["dropout"])
    ...
    if config["dset"] == 'test':
        ...
        for i, data in enumerate(data_loader_dev, 0):
            ...
            filename = data[3]
            emb = model(node_sets, mask, image, 1000)
...
def main(datadir, outdir, cashedir, bestmodelrayinfo):
    ...
    savedir = os.path.join(datadir, 'XGBoosterfeat', 'taskA')
    modeldir = os.path.join(outdir, 'multi', 'model')
    ...
    for dset in ['train', 'val', 'test']:
        ...
        if dset == 'train':
            ...
            datadict = testdict
        dataset = BERTweetevaldatasetclass(test_file=datadict,
                                           dset=dset,
                                           device=device,
                                           max_len=max_len,
                                           npdatadir=npdatadir)
