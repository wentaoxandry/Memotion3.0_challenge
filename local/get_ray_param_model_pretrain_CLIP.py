            ...
            for fd in folderlists:
                if fd.startswith('training'):
                    ...
                    for i in range(len(lines)):
                        ...
                        keepdict.update({tempdict['loss']: {}})
                        keepdict[tempdict['loss']].update({'lr': tempdict["config"]["lr"]})
                        keepdict[tempdict['loss']].update({'weight_decay': tempdict["config"]["weight_decay"]})
                        keepdict[tempdict['loss']].update({'savedir': os.path.join(subfolderdir,
                                                                                       "checkpoint_00000" + str(i),
                                                                                       "checkpoint")})

            keepdict = collections.OrderedDict(sorted(keepdict.items(), reverse=True))
    best_model_info = keepdict.get(list(keepdict)[-1])
    return list(keepdict)[-1], best_model_info['lr'], best_model_info['weight_decay'],  best_model_info['savedir']
