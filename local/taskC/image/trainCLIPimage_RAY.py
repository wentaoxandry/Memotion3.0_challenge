...
def training(config, dataset=None):
    ...
    model = CLIPimage(config["MODELtext"], config["cachedir"], type=config["type"])
    ...
    if config["pretraineddir"] == None:
        ...
        #            k.replace('clip.', 'clipimage.') in self_state and not k.replace('clip.', 'clipimage.').startswith('linear')}
        #loaded_state = {k.replace('clip.', 'clipimage.'): v for k, v in loaded_state.items() if
