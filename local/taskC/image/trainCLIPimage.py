...
def training(config, dataset=None):
    ...
    model = CLIPimage(config["MODELtext"], config["cachedir"], config["dropout"], type=config["type"])
    ...
    if config["pretraineddir"] == None:
        ...
        #loaded_state = {k.replace('module.clip.', 'clipimage.'): v for k, v in loaded_state.items() if
        #            k.replace('module.clip.', 'clipimage.') in self_state}
        loaded_state = {k: v for k, v in loaded_state.items() if not k.replace('module.', '').startswith('linear')}
        #loaded_state = {k.replace('module.clip.', 'clipimage.'): v for k, v in loaded_state.items() if
        #            k.replace('module.clip.', 'clipimage.') in self_state and not k.replace('module.clip.', 'clipimage.').startswith('module.linear')}         
...
def main(datadir, savedir, cashedir, type, bestmodelinfo):
    ...
    resultsdir = os.path.join(savedir, 'image', type, 'results')
