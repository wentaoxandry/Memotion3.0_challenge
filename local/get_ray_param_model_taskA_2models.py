...
def find_ray(sourcedir):
            ...
            for fd in folderlists:
                if fd.startswith('training'):
                    ...
                    for i in range(len(lines)):
                        ...
                        keepdict[tempdict['accuracy']].update({'dropout': tempdict["config"]["dropout"]})
    ...
    return [(list(keepdict)[0], best_model_info['lr'], best_model_info['dropout'], best_model_info['weight_decay'], best_model_info['batch_size'], best_model_info['savedir']),
            (list(keepdict)[1], second_model_info['lr'], second_model_info['dropout'], second_model_info['weight_decay'],
             second_model_info['batch_size'], second_model_info['savedir'])]



'''sourcedir = './../RAY_results/taskA_train_text_RAY'
find_ray(sourcedir)'''
