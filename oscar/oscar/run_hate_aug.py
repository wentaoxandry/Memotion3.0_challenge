...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, dataset, tokenizer, args, split='train', is_train=True):
        """
    ...
    def __getitem__(self, index):
        fileid = self.data_keys[index]
        featuredir = self.data[fileid]['features']
        boxesdir = self.data[fileid]['boxes']
        image_h = self.data[fileid]['image_h']
        image_w = self.data[fileid]['image_w']
        ...
        od_labels = ' '.join(self.data[fileid]['labels'])

        example = self.tensorize_example(caption, feature, text_b=od_labels)
        label = int(self.data[fileid]['label'])

        return index, list(example), label
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (args.save_steps > 0 and global_step % args.save_steps == 0) or \
                    ...
                    if args.evaluate_during_training:
                        ...
                        test_result, test_acc, test_auroc = test(args, model, val_dataset)
...
def test(args, model, eval_dataset):
    ...
    for step, batch in enumerate(eval_dataloader):
        batch = tuple(t.to(args.device) for t in batch[: -1])
        indexs = [i + id for i in range(len(batch[-1]))]
        id = id + len(batch[-1])
        ...
        with torch.no_grad():
            ...
            batch_score = compute_score_with_logits(logits, inputs['labels']).sum()
            batch_acc = batch_score.item() / (args.eval_batch_size)
            global_acc += batch_acc
    ...
    auroc = roc_auc_score(np.asarray(labels), np.asarray(scores))

    return results, acc, auroc
...
def main():
    ...
    parser.add_argument("--data_dir", default='dataset_aug', type=str, required=False,
    ...
    traindir = os.path.join(args.data_dir, 'train.json')
    testdir = os.path.join(args.data_dir, 'test.json')

    #deletelist = ["94528", "10463", "41309", "57864"]
    ...
    if args.do_train:
        ...
        if args.evaluate_during_training:
            val_dataset = RetrievalDataset(testdata, tokenizer, args, 'minival', is_train=False)
        else:
            val_dataset = None
