...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, dataset, tokenizer, args, split='train', is_train=True):
        ...
        self.datadir = os.path.join(args.data_dir, 'memotion3feat')
...
def test(args, model, eval_dataset, checkpoint_dir):
    ...
    acc = compute_f1(predict, labels)
    with open(os.path.join(checkpoint_dir, str(acc)[:6] + ".json"), 'w',
              encoding='utf-8') as f:
...
def main():
    ...
    if args.do_train:
        ...
        #print(config)
        #csdklc
        loaded_state = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path)).state_dict()
